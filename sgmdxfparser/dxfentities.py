...
# Created: 2016-04-17
# Updated: 2019-06-06 - Special modifications added by SIGMOE (Etienne MORO) 
# License: MIT License
...
class Text(DXFEntity):
    ...
    def setup_attributes(self, tags):
        ...
        self.set_default_extrusion()
        # Added by SIGMOE
        # Determine a global justification attribute (like attachment_point for MText)
        self.attachment_point = hvalign_to_attpt[str(self.halign)+str(self.valign)]
...
class Attrib(Text):
    ...
    def setup_attributes(self, tags):
        for code, value in super(Attrib, self).setup_attributes(tags):
            ...
            # Added by SIGMOE
            # The valign code is 74 for ATTRIB, whereas it's 73 for TEXT
            elif code == 74:
                self.valign = value
            else:
                yield code, value
...
class LWPolyline(DXFEntity):
    ...
    def setup_attributes(self, tags):
        ...
        if self.const_width != 0.:
            self.width = []
        # Added by SIGMOE
        # Special case if there is an extrusion vector code
        if self.extrusion:
            nwpoints = []
            for pts in self.points:
                nwpoint = ocs_2_wcs_xy(pts, self.elevation, self.extrusion)
                nwpoints.append(nwpoint)
            self.points = []
            self.points = list(nwpoints)
        self.set_default_extrusion()
...
class Hatch(DXFEntity):
    def __init__(self):
        ...
        self.points = []
        self.bulge = []
        # circlepath define if the border is a polyline (0) or a circle (1)
        self.circlepath = []
        self.border_type = []
        self.nb_borders = 0
        self.hatch_type = ''
        self.hatch_angle = 0.
        self.hatch_scale_spacing = 0.
    ...
    def setup_attributes(self, tags):
        ...
        for code, value in super(Hatch, self).setup_attributes(tags):
            if code == 2:
                self.hatch_type = value
            if code == 91:
                self.nb_borders = value
                startBorders = True
            elif code == 93:
                nbBordPts = value
                bulgeOk = True
                startNwborder = True
                # Case of a circle path
                if nbBordPts == 1:
                    self.circlepath.append(1)
                    bulgeOk = False
                else:
                    self.circlepath.append(0)
            elif code == 92:
                # Test the kind of boundary (polyline or multi-edge boundary)
                if bool(value & 2):
                    polyBdr = True
                else:
                    polyBdr = False
                self.border_type.append(value)
            elif code == 72:
                if not polyBdr:
                    edgeType = value
                else:
                ...
                if startBorders:
                    if startNwborder:
                        if not polyBdr:
                            if edgeType in (2, 3, 4):
                                edgeCenter = value
                            else:
                               eltPath.append(value) 
                        else:
                            edgeStart = value
                            edgeEnd = value
                            bordPoints.append(value)
                            if not bulgeOk:
                                bordBulges.append(0.)
                            cntBordPts += 1
                            if cntBordPts == nbBordPts:
                                startNwborder = False
                                cntBordPts = 0
                                self.points.append(bordPoints)
                                bordPoints = []
                                if not bulgeOk:
                                    bordBulges.append(0.)
                                    self.bulge.append(bordBulges)
                                    bordBulges=[]
                            bulgeOk = False
            elif code == 11:
                if edgeType == 1:
                    # edgeEnd = value
                    eltPath.append(value)
                    # Add a bulge 0 for a Line path
                    eltPath.append(0.)
                    lstEltPath.append(eltPath)
                    eltPath = []
            elif code == 40:
                # Special case of a circle as the path
                # In this case the bulge = the circle radius
                if nbBordPts == 1:
                    self.bulge.append(value)
                    bulgeOk = True
                # Case of a an arc edge
                else:
                    edgeRadius = value
            elif code == 50:
                edgeStartAngle = value
            elif code == 51:
                edgeEndAngle = value
            elif code == 73:
                # Special case of an arc edge
                if not polyBdr:
                    counterclockwiseFlag = value
                    if startBorders:
                        # Calculate a bulge for the arc
                        edgeBulge = bulgeFromArc(edgeStartAngle, edgeEndAngle, counterclockwiseFlag)
                        # Calculate the new edgeStart (first point of the arc)
                        edgeStart = polarPointDg(edgeCenter, edgeRadius, edgeStartAngle)
                        # Calculate the new edgeEnd (last point of the arc)
                        if not counterclockwiseFlag:
                            edgeEndAngle = (360 - edgeEndAngle) % 360
                        edgeEnd = polarPointDg(edgeCenter, edgeRadius, edgeEndAngle)
                        eltPath.append(edgeStart)
                        eltPath.append(edgeEnd)
                        eltPath.append(edgeBulge)
                        lstEltPath.append(eltPath)
                        eltPath = []
                        cntBordPts += 1
            elif code == 42:
                if startBorders :
                    bordBulges.append(value)
                    bulgeOk = True
                    if not startNwborder :
                        self.bulge.append(bordBulges)
                        bordBulges=[]
            elif code == 52:
                self.hatch_angle = value
            elif code == 41:
                self.hatch_scale_spacing = value
            # End of a path
            elif code == 97:
                if not polyBdr:
                    startNwborder = False
                    ...
                    bordPoints = []
                    bordBulges = []
                    for idxElt, elt in enumerate(lstEltPath):
                        if idxElt == 0:
                            bordPoints.append(elt[0])
                            bordPoints.append(elt[1])
                        else:
                            bordPoints.append(elt[1])
                        bordBulges.append(elt[2])
                    lstEltPath = []
                    self.points.append(bordPoints)
                    bordPoints = []
                    self.bulge.append(bordBulges)
...
# Added by SIGMOE
# Cross vector calculation with 2 matrices
def cross(u, v):
    return [u[1]*v[2]-u[2]*v[1], u[2]*v[0]-u[0]*v[2], u[0]*v[1]-u[1]*v[0]]

# Added by SIGMOE
# Transform a 2D point from Object Coordinate Sytem to WCS
def ocs_2_wcs_xy(xy_point, elevation, n_vector):
    # Add the Z 
    xyz_point = list(xy_point)
    if len(xyz_point) < 3:
        xyz_point.append(elevation)
    # This is the Arbitrary Axis Algorithm (DXF)
    if abs(n_vector[0]) < 1/64 and abs(n_vector[1]) < 1/64:
        ax = cross([0,1,0], n_vector)
    else:
        ax = cross([0,0,1], n_vector)
    ay = cross(n_vector, ax)
    az = n_vector
    xyz = [ xyz_point[0]*ax[0] + xyz_point[1]*ay[0] + xyz_point[2]*az[0],
            xyz_point[0]*ax[1] + xyz_point[1]*ay[1] + xyz_point[2]*az[1],
            xyz_point[0]*ax[2] + xyz_point[1]*ay[2] + xyz_point[2]*az[2]
          ]
    nwpt = (xyz[0], xyz[1])
    return nwpt
...
def bulgeFromArc(arcStartAngle, arcEndAngle, counterclockwise):
    angle = (arcEndAngle - arcStartAngle) % 360
    bulge = math.tan(math.radians(angle) / 4.0)
    if not counterclockwise:
        bulge = - bulge
    return bulge
    
# Added by SIGMOE
# Calculate the coordinates of a point over an arc from the center of the arc,
# the radius, and the angle 
def polarPointDg(center, radius, angle):
    x = center[0] + abs(radius) * math.cos(math.pi * angle / 180)
    y = center[1] + abs(radius) * math.sin(math.pi * angle / 180)
    return (x, y)
    
from qgis.core import QgsMessageLog

# Show the debug messages
def debugMessage(debugOnOff, msgStr, msgListvar):
    if debugOnOff == 'DEBUG':
        msg = msgStr % msgListvar
